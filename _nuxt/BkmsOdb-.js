import{k as a,s as o}from"./jmU4ISgN.js";const r=[...a,...o],i=Object.fromEntries(r.map(e=>[e.id,e])),t=e=>{const s=i[e];if(!s)throw new Error(`Speaker not found: ${e}`);return s},n=[{id:"nurse",type:"keynote",title:"Keynote",speaker:t("nurse"),abstract:"詳細は後日公開します。"},{id:"harukasan",type:"session",title:"「照らす技術」をRubyで照らす",speaker:t("harukasan"),abstract:`ステージを華やかに彩る照明や、演者の影でせわしなく動き回るムービングライト。これらを裏側で制御する共通言語が「DMX512」という通信プロトコルです。本セッションでは、ふだん裏方に徹している「ステージを照らす技術」にスポットライトを当て、そのプロトコルの実装を紹介します。
...
セッションの最後には、実際に会場へ持ち込んだムービングライトを、Rubyコードを用いてリアルタイムに制御するデモンストレーションを実施する予定です。Rubyが多様なプロトコルを話すことで広がる、新たな表現の可能性をぜひ体感してください。`},{id:"aim2bpg",type:"session",title:"レールロード図で照らすOnigmoのバグ —— Narakuが変えるRuby正規表現の未来",speaker:t("aim2bpg"),abstract:`このトークの価値は、RubyKaigi 2026で発表されたばかりのNarakuという、非常にタイムリーなプロジェクトを、具体的なバグ事例と可視化ツールを通して紹介できる点にあります。
...
Rubreeは2022年にフィヨルドブートキャンプの卒業制作として開発を開始し、RubyKaigi 2025をきっかけに開発を再開しました。現在も継続的に運用しており、Ruby Weekly #777にも掲載されています。発表中のライブデモにも十分な説得力を持たせられると考えています。`},{id:"se4weed",type:"session",title:"torikago - Ruby::Boxで照らすモジュラモノリスの実行境界",speaker:t("se4weed"),abstract:`packwerkやRails::Engineを使うと、Railsのモジュラモノリスに構造上の境界を作ることができます。依存関係を明確にし、「どこに影響が出るか分からずコードを触れない」といった大規模なコードベースの困りごとを減らすうえで、とても有効です。
